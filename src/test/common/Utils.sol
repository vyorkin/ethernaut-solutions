...
contract Utils is DSTest {
    ...
    function getNamedAddress(string calldata name)
        external
        pure
    ...
    /// @notice Creates users with initial balance.
    ...
    /// @notice Moves block.number forward by a given number of blocks.
    function mineBlocks(uint256 count) external {
        uint256 target = block.number + count;
        vm.roll(target);
    }

    /// @notice Moves forward by a given number of seconds.
    function mineTime(uint256 secs) external {
      uint256 target = block.timestamp + secs;
      vm.warp(target);
    }
}
